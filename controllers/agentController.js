...
export const getAgentPerformance = async (req, res) => {
  try {
    ...
    const avgRating = agent.Ratings.reduce((sum, rating) => sum + rating.score, 0) / agent.Ratings.length;
...
export const getAllAgents = async (req, res) => {
  try {
    const agents = await Agent.findAll({
      attributes: ['id', 'firstName', 'lastName', 'email', 'isActive', 'rating', 'ticketType'],
      order: [['rating', 'DESC']]
    });
    res.json(agents);
  } catch (error) {
