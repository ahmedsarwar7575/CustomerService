...
function classifyIssue(t=""){t=t.toLowerCase();if(/(bill|payment|invoice|refund|charge|card)/.test(t))return"billing";if(/(login|password|verify|otp|lock|unlock|2fa|account)/.test(t))return"account";if(/(bug|error|crash|fail|broken|not working|issue)/.test(t))return"technical";if(/(buy|pricing|quote|plan|subscription|upgrade|downgrade)/.test(t))return"sales";if(/(support|help|question|how to)/.test(t))return"support";return"other"}
function toQAPairs(tr=[]){const out=[];let q=null;for(const m of tr){if(m.role==="user"){if(q)out.push({q,a:""});q=m.text||""}else if(m.role==="assistant"){if(q!==null){out.push({q,a:m.text||""});q=null}}}if(q)out.push({q,a:""});return out}
...
function buildSessionUpdate(){
  return{type:"session.update",session:{turn_detection:{type:"server_vad",threshold:0.6,prefix_padding_ms:200,silence_duration_ms:300},input_audio_format:"g711_ulaw",output_audio_format:"g711_ulaw",voice:REALTIME_VOICE,instructions:SYSTEM_MESSAGE,modalities:["text","audio"],temperature:0.2}};
}

export function attachMediaStreamServer(server){
  const wss=new WebSocketServer({server,path:"/media-stream"});
  wss.on("connection",(connection)=>{
    let streamSid=null,callSid=null,latestMediaTimestamp=0,lastAssistantItem=null,markQueue=[],responseStartTimestampTwilio=null,textBuffer="",finalJsonString=null,printed=false;
    const openAiWs=createOpenAIWebSocket();
    const initializeSession=()=>{openAiWs.send(JSON.stringify(buildSessionUpdate()))};
    const handleSpeechStartedEvent=()=>{
      if(markQueue.length>0&&responseStartTimestampTwilio!=null){
        const elapsed=latestMediaTimestamp-responseStartTimestampTwilio;
        if(lastAssistantItem){openAiWs.send(JSON.stringify({type:"conversation.item.truncate",item_id:lastAssistantItem,content_index:0,audio_end_ms:Math.max(0,elapsed)}))}
        connection.send(JSON.stringify({event:"clear",streamSid}));
        markQueue=[];lastAssistantItem=null;responseStartTimestampTwilio=null;
      }
    };
    const sendMark=()=>{if(!streamSid)return;connection.send(JSON.stringify({event:"mark",streamSid,mark:{name:"responsePart"}}));markQueue.push("responsePart")};

    openAiWs.on("open",()=>setTimeout(initializeSession,100));
    openAiWs.on("message",(data)=>{try{
    ...
    function emitFinalOnce(){
      ...
      const fullConvo=Array.isArray(raw?.transcript)?toQAPairs(raw.transcript):[];
      console.log(JSON.stringify({name,email,summary,isIssueResolved,issue,fullConvo}));
      printed=true;
    }

    connection.on("message",(message)=>{try{
      const data=JSON.parse(message);
      switch(data.event){
        case"start":
          streamSid=data.start.streamSid;callSid=data.start.callSid||null;responseStartTimestampTwilio=null;latestMediaTimestamp=0;break;
        case"media":
          latestMediaTimestamp=Number(data.media.timestamp)||latestMediaTimestamp;
          if(openAiWs.readyState===WebSocket.OPEN)openAiWs.send(JSON.stringify({type:"input_audio_buffer.append",audio:data.media.payload}));
          break;
        case"mark":
          if(markQueue.length)markQueue.shift();break;
        case"stop":
          if(openAiWs.readyState===WebSocket.OPEN){try{openAiWs.send(JSON.stringify({type:"input_audio_buffer.commit"}))}catch{}try{openAiWs.close()}catch{}}
          emitFinalOnce();
          break;
        default:break;
      }
    ...
    connection.on("close",()=>{if(openAiWs.readyState===WebSocket.OPEN){try{openAiWs.close()}catch{}}emitFinalOnce()});
  });
}
