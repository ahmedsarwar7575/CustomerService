...
`.trim();
    ...
    const safe = {
      ...
      ticket: {
        ...
        priority: ["low", "medium", "high", "critical"].includes(parsed?.ticket?.priority) ? parsed.ticket.priority : "medium",
        ticketType: ["support", "sales", "billing"].includes(parsed?.ticket?.ticketType) ? parsed.ticket.ticketType : null,
        proposedSolution: ns(parsed?.ticket?.proposedSolution),
        isSatisfied: parsed?.ticket?.isSatisfied === true ? true : parsed?.ticket?.isSatisfied === false ? false : null
      },
    ...
    const result = await sequelize.transaction(async (t) => {
      ...
      if (safe.customer.email) {
        userRecord = await User.findOne({ where: { email: safe.customer.email }, transaction: t });
      }
      if (!userRecord && safe.customer.phone) {
        userRecord = await User.findOne({ where: { phone: safe.customer.phone }, transaction: t });
      }
      if (!userRecord) {
        userRecord = await User.create({ name: safe.customer.name, email: safe.customer.email, phone: safe.customer.phone }, { transaction: t });
      }
