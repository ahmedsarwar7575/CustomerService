...
await connectIndex().catch(e => { console.error("[BOOT] Pinecone error", e); process.exit(1); });
...
router.get("/realtime-session", async (req, res) => {
  try {
    ...
    if (!key) return res.status(500).json({ error: "No client_secret in response", raw: json });
