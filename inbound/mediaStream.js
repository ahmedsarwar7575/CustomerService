...
const log = (lvl, tag, obj = {}) => {
  ...
  if (L < MIN_LVL) return;
  console.log(
    JSON.stringify({ ts: new Date().toISOString(), lvl, tag, ...obj })
  );
};
...
const peek = (buf, n = 180) => {
  try {
    return (Buffer.isBuffer(buf) ? buf.toString("utf8") : String(buf)).slice(
      0,
      n
    );
  } catch {
...
async function retrieveAndRerank(queries, { topK, minScore }) {
  ...
  const bags = await Promise.all(
    queries.map((q) =>
      semanticSearch(q, { topK: perQ, minScore }).catch(() => [])
    )
  );
  const all = bags.flat();
  const byId = new Map();
  for (const m of all) {
    ...
    if (!p || (m.score ?? 0) > (p.score ?? 0)) byId.set(m.id, m);
  }
  const dedup = [...byId.values()].sort(
    (a, b) => (b.score ?? 0) - (a.score ?? 0)
  );
  const out = [];
  for (const m of dedup) {
    const t = m.metadata?.text || m.metadata?.chunk || m.text || "";
    const sim = out.some(
      (o) =>
        jaccard(o.metadata?.text || o.metadata?.chunk || o.text || "", t) > 0.85
    );
    if (!sim) out.push(m);
...
export async function attachMediaStreamServer(server) {
  await connectIndex().catch((e) => {
    log("error", "PINECONE_CONNECT_FAIL", { err: String(e?.message || e) });
    process.exit(1);
  });
  ...
  wss.on("connection", (connection) => {
    ...
    const openAiReady = () =>
      openAiWs.readyState === WebSocket.OPEN && sessionReady;
    ...
    const requestResponse = (reason = "") => {
      ...
      try {
        ...
        log("error", "RESP_CREATE_ERR", { err: String(e?.message || e) });
    ...
    const flushInBufToOpenAI = () => {
      ...
      for (const payload of inBuf) {
        try {
          ...
          log("error", "OPENAI_APPEND_FAIL_FLUSH", {
    ...
    openAiWs.on("close", (c, r) => {
      clearInterval(hb);
      log("warn", "OPENAI_WS_CLOSE", { code: c, reason: String(r) });
    });
    openAiWs.on("error", (e) =>
      log("error", "OPENAI_WS_ERROR", { err: String(e?.message || e) })
    );
    ...
    openAiWs.on("message", async (data, isBinary) => {
      ...
      if (msg.type === "response.function_call_arguments.done") {
        const fn = msg.name;
        const args =
          parseMsg(toolArgBuffer[fn] || "{}", false, "OPENAI_TOOL_ARGS") || {};
        delete toolArgBuffer[fn];

        if (fn === "search_kb" && args.query) {
          const qMain = String(args.query || "").trim();
          const qAlts = Array.isArray(args.queries)
            ? args.queries.filter(Boolean).map(String)
            : [];
          const topK = Number(args.topK || TOPK);
          const minScore = Number(args.minScore || RAG_MIN_SCORE);

          if (rawTranscript)
            log("info", "BEFORE_REWRITE", { t: rawTranscript });
          if (qMain) log("info", "REWRITE", { best: qMain, alts: qAlts });

          let snippets = [];
          try {
            const queries = Array.from(
              new Set([qMain, ...qAlts].filter(Boolean))
            );
            const results = await retrieveAndRerank(queries, {
              topK,
              minScore,
            });
            snippets = results.map((r) => ({
              ...
              text: String(
                r.metadata?.text || r.metadata?.chunk || r.text || ""
              ).slice(0, 1600),
            }));
            log("info", "PINECONE_HITS", {
              n: snippets.length,
        ...
        for (const out of outs) {
          if (out?.role === "assistant") {
            const part = Array.isArray(out.content)
              ? out.content.find((c) => typeof c?.transcript === "string")
                  ?.transcript ??
                out.content.find((c) => typeof c?.text === "string")?.text
              : null;
            const a = (part || "").trim();
            if (a) {
              const q = rawTranscript || "not specified";
              qaPairs.push({ q, a });
    ...
    connection.on("message", async (message, isBinary) => {
      ...
      switch (data.event) {
        ...
        case "start": {
          ...
          if (
            ...
            try {
              ...
              log("warn", "TWILIO_RECORDING_FAIL", {
                err: String(e?.message || e),
              });
            }
          }
        ...
        case "media": {
          ...
          if (!payload) return;
          if (!openAiReady()) {
            if (inBuf.length >= INBUF_MAX) {
              inBuf.shift();
              log("debug", "INBUF_DROP_OVERFLOW", { size: inBuf.length });
            }
            inBuf.push(payload);
    ...
    connection.on("close", async () => {
      ...
      try {
        ...
        log("warn", "SUMMARY_FAIL", { err: String(e?.message || e) });
      }
    });

    setTimeout(() => {
      if (!sessionReady || !streamReady)
        log("warn", "NOT_READY_8S", { sessionReady, streamReady });
    }, 8000);
  });
}
